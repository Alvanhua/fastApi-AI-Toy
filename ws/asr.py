...
import time
import websockets
from .config import  CONNECTIONS,CLIENTS,TOKEN,APPID
...
class AsrWsClient:
    ...
    async def send_full_request(self, mac_address,get_config):
        ...
        async with websockets.connect(self.ws_url, extra_headers=header, max_size=1000000000,ssl=ssl_context) as ws:
            ...
            while True:
                ...
                # print("取")
                # if word_times>=20:
                #     # print("识别结束,告诉单片机识别结束,停止麦克风")
                #     # 告诉单片机停止麦克风
                #     await CLIENTS[mac_address].send("close_mic")
                #     #清空队列
                #     CONNECTIONS[mac_address]= asyncio.Queue()
                #
                #     last=True
                # try:
                #     # 尝试从队列中获取元素，等待时间为0.1秒
                #     chunk = await asyncio.wait_for(CONNECTIONS[mac_address].get(), timeout=0.1)
                #     if len(chunk)<100:
                #         print(chunk)
                #     # if chunk==b"0x04":
                #     #     print("六"*60)
                # except asyncio.TimeoutError:
                #     # 如果在0.1秒内没有获取到元素，则打印队列为空
                #     last = True
                #     # print("队列为空")
                chunk = await asyncio.wait_for(CONNECTIONS[mac_address].get(), timeout=0.1)
                if chunk==b'\xd3\x1d8':
                ...
                res = await ws.recv()
                result = parse_response(res)
                # tt=result['payload_msg'].get('result')
                # if tt:
                #     text1 = tt[0]['text']
                #     new_word_amount=len(text1)
                #     if new_word_amount==word_amount:
                #         word_times += 1
                #     else:
                #         word_amount=new_word_amount
                #         word_times=0
                # print("asr流式响应:",text1)
                # print("文字数量:",word_amount)
                # print("文字次数:",word_times)
                if 'payload_msg' in result and result['payload_msg']['code'] != self.success_code:
