...
import gzip
import copy
from .config import CLIENTS,APPID,TOKEN
...
async def test_submit(text, mac_address, tts_config, wake_up=False):
    uid = str(uuid.uuid4())
    ...
    request_json = {
        ...
        "audio": {
            "voice_type": tts_config.get('voice_type', 'BV064_streaming'),
            "encoding": "pcm",
            "speed_ratio": tts_config.get('speed_ratio', 1.0),
            "volume_ratio": tts_config.get('volume_ratio', 2.0),
    ...
    async with websockets.connect(api_url, extra_headers=header, ssl=ssl_context, ping_interval=None) as ws:
    ...
    if message_type == 0xb:  # audio-only server response
        ...
        while True:
            ...
            if chunk:
                # if wake_up:
                #     chunk=b'\x01'+chunk
                await CLIENTS[mac_address].send_bytes(chunk)
            else:
                break
